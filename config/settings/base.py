...
# Static & Media files
# -----------------------------------------------------------------------
# نکته حیاتی: STATIC_URL و MEDIA_URL باید با '/' شروع شوند.
# اگر بدون '/' باشند (مثلاً 'static/' به‌جای '/static/')، مرورگر آن‌ها را
# به‌عنوان URL نسبی به مسیر صفحه‌ی جاری تفسیر می‌کند، نه نسبت به ریشه‌ی سایت.
# یعنی در صفحه‌ی اصلی (/) کار می‌کرد، اما در هر صفحه‌ی یک‌پله‌عمیق‌تر مثل
# صفحه‌ی عمومی رستوران (/<slug>/) همه‌ی لینک‌های استاتیک و عکس‌ها به آدرس
# اشتباه /<slug>/static/... می‌رفتند و ۴۰۴ می‌گرفتند — دقیقاً همان چیزی که
# باعث شد صفحه‌ی عمومی بدون استایل/عکس دیده شود.
STATIC_URL = '/static/'
